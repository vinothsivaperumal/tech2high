...
function hasSmtpConfig() {
    return Boolean(env_1.env.SMTP_HOST.trim() && env_1.env.SMTP_USER.trim() && env_1.env.SMTP_PASS.trim());
}
function createTransport() {
    ...
    const options = {
        ...
        ...(env_1.env.SMTP_USER.trim() && env_1.env.SMTP_PASS.trim()
            ? {
                auth: {
...
async function sendRegistrationEmail(params) {
    ...
    const loginUrl = `${env_1.env.CORS_ORIGIN.replace(/\/$/, "")}/login`;
