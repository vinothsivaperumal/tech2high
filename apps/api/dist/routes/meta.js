...
const countryQuerySchema = zod_1.z.object({
    country: zod_1.z.string().trim().min(1)
});
const cityQuerySchema = zod_1.z.object({
    country: zod_1.z.string().trim().min(1),
...
exports.metaRouter.get("/countries", async (_req, res) => {
    try {
        if (countriesCache.loaded) {
            res.json({ countries: countriesCache.values });
            return;
        }
        const payload = await fetchJson(`${GEO_API_BASE}/countries/positions`);
...
exports.metaRouter.get("/states", async (req, res) => {
    ...
    try {
        ...
        const payload = await fetchJson(`${GEO_API_BASE}/countries/states/q?country=${encodeURIComponent(country)}`);
...
exports.metaRouter.get("/cities", async (req, res) => {
    ...
    try {
        ...
        const payload = await fetchJson(`${GEO_API_BASE}/countries/state/cities/q?country=${encodeURIComponent(country)}&state=${encodeURIComponent(state)}`);
        if (payload.error || !Array.isArray(payload.data)) {
...
exports.metaRouter.get("/detect-location", async (_req, res) => {
    try {
