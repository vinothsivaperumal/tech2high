...
authRouter.post("/refresh", async (req, res) => {
    ...
    authRouter.post("/refresh", async (req, res) => {
        ...
        try {
            const payload = (0, refreshToken_1.verifyRefreshToken)(refreshToken);
            // Optionally: check user still exists and is active
            const result = await client_1.pool.query(`SELECT ${userSelectFields} FROM users WHERE id = $1`, [payload.id]);
            if (!result.rowCount) {
...
`;
    ...
    export const authRouter = (0, express_1.Router)();
    ...
    `, [
        ...
        const refreshToken = (0, refreshToken_2.signRefreshToken)({
            id: user.id,
            email: user.email,
            role: user.role
        });
    ...
    authRouter.patch("/me", auth_1.requireAuth, async (req, res) => {
